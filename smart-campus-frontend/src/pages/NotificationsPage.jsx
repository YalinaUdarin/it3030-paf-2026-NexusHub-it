...
const PREFERENCE_LABELS = [
  ...
  },
  {
    ...
    icon: '🔄',
    description: 'When your ticket status is updated',
  },
  {
    key: 'ticketAssigned',
          ...
          <div className="space-y-2">
            {PREFERENCE_LABELS.map(({ key, label, icon, description }) => (
              ...
              >
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow ${preferences[key] ? 'translate-x-6' : 'translate-x-1'}`}
                  />
                </button>
              </div>
            ))}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <p
                    className={`text-sm font-medium ${!n.read ? 'text-blue-900' : 'text-gray-900'}`}
                  >
                    {n.title}
                  </p>
                  {!n.read && (

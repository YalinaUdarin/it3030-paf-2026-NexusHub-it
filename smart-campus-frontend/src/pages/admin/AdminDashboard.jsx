...
export default function AdminDashboard() {
  ...
  const bookingStatusData = ['PENDING', 'APPROVED', 'REJECTED', 'CANCELLED']
    .map((s) => ({
      name: s,
      value: stats.bookings.filter((b) => b.status === s).length,
    }))
    .filter((d) => d.value > 0);

  const ticketStatusData = ['OPEN', 'IN_PROGRESS', 'RESOLVED', 'CLOSED'].map(
    (s) => ({
      name: s.replace('_', ' '),
      value: stats.tickets.filter((t) => t.status === s).length,
    }),
  );

  const ticketPriorityData = ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']
    .map((p) => ({
      name: p,
      value: stats.tickets.filter((t) => t.priority === p).length,
    }))
    .filter((d) => d.value > 0);
  ...
  stats.bookings.forEach((b) => {
    if (b.resourceName)
      resourceBookingCount[b.resourceName] =
        (resourceBookingCount[b.resourceName] || 0) + 1;
  });
  ...
  const approvalRate =
    stats.bookings.length > 0
      ? Math.round(
          (stats.bookings.filter((b) => b.status === 'APPROVED').length /
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      icon: HiCalendar,
      color: 'orange',
      sub: `${stats.bookings.filter((b) => b.status === 'PENDING').length} pending`,
    },
    {
      label: 'Total Tickets',
      ...
      icon: HiTicket,
      color: 'purple',
      sub: `${stats.tickets.filter((t) => t.status === 'OPEN').length} open`,
    },
    {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Campus operations overview and analytics
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Most frequently reserved facilities
          </p>
        ...
        <div className="card">
          ...
          <p className="text-xs text-gray-400 mb-4">
            When resources are booked most often
          </p>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={peakHours}
            ...
            >
              ...
              <Bar dataKey="bookings" radius={[6, 6, 0, 0]} barSize={20}>
                {peakHours.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={entry.bookings > 0 ? '#10b981' : '#e2e8f0'}
                  />
                ))}
              </Bar>
            </BarChart>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Distribution by approval state
          </p>
          {bookingStatusData.length === 0 ? (
            ...
            <div className="space-y-3">
              {bookingStatusData.map((d) => {
                const total = bookingStatusData.reduce(
                  (s, x) => s + x.value,
                  0,
                );
            ...
            {[
              {
                ...
                value: stats.bookings.filter((b) => b.status === 'PENDING')
                  .length,
              ...
              {
                label: 'Open Tickets',
                value: stats.tickets.filter((t) => t.status === 'OPEN').length,
                color: 'text-blue-600',
                bg: 'bg-blue-50',
              },
              {
                label: 'Critical Tickets',
                value: stats.tickets.filter((t) => t.priority === 'CRITICAL')
                  .length,
                color: 'text-red-600',
                bg: 'bg-red-50',
              },
              {
                label: 'In Progress',
                value: stats.tickets.filter((t) => t.status === 'IN_PROGRESS')
                  .length,
              ...
              },
              {
                ...
                value: stats.tickets.filter((t) => t.status === 'RESOLVED')
                  .length,
              ...
              },
              {
                ...
                value: stats.bookings.filter((b) => b.status === 'APPROVED')
                  .length,
                color: 'text-green-600',
                bg: 'bg-green-50',
              },
              ...
              <div
                key={label}
                className={`flex items-center justify-between px-3 py-2 rounded-xl ${bg}`}
              >
                <span className="text-xs font-medium text-gray-600">
                  {label}
                </span>

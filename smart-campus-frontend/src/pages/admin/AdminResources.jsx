...
export default function AdminResources() {
  ...
  const removeWindow = (i) =>
    setForm((f) => ({
      ...f,
      availabilityWindows: f.availabilityWindows.filter((_, idx) => idx !== i),
    }));
  const updateWindow = (i, field, value) =>
    setForm((f) => ({
      ...f,
      availabilityWindows: f.availabilityWindows.map((w, idx) =>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Create and manage campus facilities and assets
          </p>
        </div>
        <button
          onClick={openCreate}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="font-bold text-gray-900 text-lg">
                {modal === 'create' ? 'Add New Resource' : 'Edit Resource'}
              </h3>
              <button
                ...
                {form.availabilityWindows.map((w, i) => (
                  <div key={i} className="flex gap-2 mb-2 items-center">
                    <select
                      value={w.dayOfWeek}
                      onChange={(e) =>
                        updateWindow(i, 'dayOfWeek', e.target.value)
                      }

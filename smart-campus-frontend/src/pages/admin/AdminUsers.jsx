        ...
        <div className="card overflow-hidden p-0">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['User', 'Email', 'Roles', 'Status', 'Joined', 'Actions'].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <img
                          src={
                            u.picture ||
                            `https://ui-avatars.com/api/?name=${u.name}&size=32`
                          }
                          alt={u.name}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1 flex-wrap">
                        {u.roles?.map((r) => (
                          <span
                            key={r}
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${roleColor[r] || 'bg-gray-100 text-gray-700'}`}
                          >
                            {r}
                          </span>
                        ))}
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${u.enabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                      >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex items-center gap-3 mb-6 p-3 bg-gray-50 rounded-xl">
              <img
                src={
                  roleModal.picture ||
                  `https://ui-avatars.com/api/?name=${roleModal.name}`
                }
                alt={roleModal.name}
            ...
            <div className="space-y-2 mb-6">
              {ALL_ROLES.map((role) => (
                <label
                  key={role}
                  className="flex items-center justify-between p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-blue-300 hover:bg-blue-50 transition-colors"
                >
                  <div>
